...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self,
        ...
        for i in range(b):
            class_m[i], loc_m[i] = match(iou_threshold=self.iou_threshold,
                                         class_targets=class_targets[i],
                                         loc_targets=loc_targets[i],
        ...
        # compute max confidence across batch for hard negative mining
        batch_conf = class_preds.view(-1, self.class_count)
        gathered_classes = batch_conf.gather(1, class_targets.view(-1, 1))
        class_loss = self.log_sum_exp(batch_conf) - gathered_classes
