...
def decode(loc_pred,
           ...
           variances):
    boxes = torch.cat((
        anchors[:, :2] + loc_pred[:, :2] * variances[0] * anchors[:, 2:],
        anchors[:, 2:] * torch.exp(loc_pred[:, 2:] * variances[1])), 1)
    boxes[:, :2] -= boxes[:, 2:] / 2
    boxes[:, 2:] += boxes[:, :2]
...
def match(iou_threshold,
    ...
    matched = loc_targets[best_object_i]
    loc_match = encode(matched, anchors, variances)

    class_match = class_targets[best_object_i] + 1
    class_match[best_object_iou < iou_threshold] = 0
